{ config, pkgs, ... }:
{
  ...
  programs.zsh = {
    enable = true;
    enableAutosuggestions = true;  # ✅ Fixed: use enableAutosuggestions instead of autosuggestion.enable
    enableSyntaxHighlighting = true;
    
    # oh-my-zsh configuration - CORRECTED
    ohMyZsh = {  # ✅ Fixed: capital 'M' in MyZsh
      enable = true;
      theme = "robbyrussell";
      plugins = [
        "git"
    ...
    # zsh-autoenv (if available in your nixpkgs)
    # Note: zsh-autoenv might need to be handled differently in Home Manager
    initExtra = ''
      # Autoenv-like functionality
      if [ -f ".autoenv_leave" ]; then
