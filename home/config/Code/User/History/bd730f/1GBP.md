...
```
 ...
 ```
  ...
  ./main
```
Or run `./main_with_random` to see hwo the weighing scales work when the adc register is assuming there is an ISR working
# Architecture 
...
I added init programs but did not end up using them, was there to show the depth of setup and tailoring.

## Unit testing
The unit tests all function bar get_loads -- as that is a wrapper function. 
The minium required tests are completed in get_weights but other tests for printing and displays are done also